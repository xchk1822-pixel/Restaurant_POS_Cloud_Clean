...
- POS frontend experience pass: completed-order publishing now queues the final `pos_orders` write in the background after local completion state, reducing weak-network button blocking while preserving complete-time stock deduction. The POS overview table canvas and right order list were polished, with `全部` moved beside `订单列表` and only `堂食`/`打包`/`外卖` left in the lower filter row.
...
- DataManager now refuses bare business-cache reads/writes without `storeId`, so store-scoped orders, customers, expenses, purchases, handovers, employees, menu items, and inventory cannot fall back to global localStorage keys.
- DataManager awaited saves now also wait for Firestore sync tasks before notifying module listeners, so `await dataManager.saveData(...)` no longer hides unfinished cloud writes.
