...
- POS terminal order sync hotfix: cancel and complete actions now publish terminal order state directly to `pos_orders` before local state changes, covering dine-in clear-table completion plus takeout/delivery completion. Deployed to Firebase Hosting in commit `pending`.
