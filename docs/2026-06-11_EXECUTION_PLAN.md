...
- POS terminal order merge hotfix: a cloud `completed`/`cancelled` order now overrides a local non-terminal cached copy even when the local cache has a newer `lastModified`, preventing one-device stale completed orders. Deployed to Firebase Hosting in commit `pending`.
