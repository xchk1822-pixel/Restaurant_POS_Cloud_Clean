...
本系统采用**统一数据管理服务（DataManager）**确保全系统数据的一致性、互联性和存储一致性。
...
- 通过 `DataManager` 单例访问，避免多处读写导致的不一致
- 缓存机制提高性能，同时保证与 localStorage 同步
...
| `customers` | `customers` | 客户信息 | 客户管理、数据概览 |
...
// 输出: { orders: 25, expenses: 10, purchases: 5, ... }
